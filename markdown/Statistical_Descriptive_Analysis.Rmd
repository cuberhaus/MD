...
descriptive<-function(X, nom, tabNum){
  if (!(is.numeric(X))){
    ...
    foot <- " frequency and proportion table (sorted)."
...
for (k in actives)
{
  ...
  if (name !=) {    plot(ddAfter[,k], dbPreprocessed$, pch=16, col=alpha('blue', 0.6),
                              main= paste(name, " "),
                              xlab=name, ylab='')
  
  if (is.numeric(dbPreprocessed[,k])) cat(paste("Correlation between ", name, " and  ", cor(dbPreprocessed[,k], dbPreprocessed$)))
  }
}
```
